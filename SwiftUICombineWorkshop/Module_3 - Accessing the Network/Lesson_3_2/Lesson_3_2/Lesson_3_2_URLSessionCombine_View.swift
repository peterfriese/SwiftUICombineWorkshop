/// Lesson 3.2: Use Combine to fetch data from the network
///
/// Steps:
/// 1. Rename `checkUserNameAvailable` to `checkUserNameAvailableOld`
/// 2. Create a new function `checkUserNameAvailable`, returning `AnyPublisher<Bool. Never>`
/// 3. Use `URLSession.shared.dataTaskPublisher` to fetch data
/// 4. Map the result using `map`
/// 5. Take the `do/catch` block from `checkUserNameAvailableOld` and use it inside `map`
/// 6. Handle errors using `replaceError(with):`
/// 7. Type-erase the result
/// 8. Update the call site
...
struct Lesson_3_2_URLSessionCombine_View: View {
...
struct ContentView_Previews: PreviewProvider {
    static var previews: some View {
        Lesson_3_2_URLSessionCombine_View()
    }
}
